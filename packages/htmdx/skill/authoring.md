...
```bash
npx @wix/htmdx@4.13.0 skill components    # body grammar for every component
npx @wix/htmdx@4.13.0 skill integration   # React host, registration, testing
npx @wix/htmdx@4.13.0 skill starter       # a starter artifact to copy
```
...
<html lang="en">
  <head>
    ...
    <script src="https://cdn.jsdelivr.net/npm/@wix/htmdx@4.13.0/dist/browser.js" defer></script>
  </head>
  <body>
    <!-- prettier-ignore -->
...
```bash
npx @wix/htmdx@4.13.0 lint path/to/artifact.html --strict
```
...
```bash
npx @wix/htmdx@4.13.0 lint artifact.html          # one artifact
npx @wix/htmdx@4.13.0 lint a.html b.html doc.mdx  # several files
npx @wix/htmdx@4.13.0 lint artifacts/*.html       # let the shell expand it
npx @wix/htmdx@4.13.0 lint artifact.html --strict --format json
```
...
artifact.html
  2:26     warning  unpinned-runtime
    runtime is not pinned to a version; a future release can change this artifact (pin @wix/htmdx@4.13.0)
...
```bash
npx @wix/htmdx@4.13.0 compile artifact.html --out snapshot.html
npx @wix/htmdx@4.13.0 components            # the whole catalog, grouped by source
npx @wix/htmdx@4.13.0 components Foldout    # purpose, body mode, props, example
```
...
```bash
npx @wix/htmdx@4.13.0 skill                 # this topic (authoring)
npx @wix/htmdx@4.13.0 skill --list          # available topics
npx @wix/htmdx@4.13.0 skill components      # one topic
npx @wix/htmdx@4.13.0 skill --full          # every topic in one stream
npx @wix/htmdx@4.13.0 skill starter > brief.html
```
